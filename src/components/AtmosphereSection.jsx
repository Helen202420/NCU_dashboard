...
const AtmosphereSection = () => {
  ...
  useEffect(() => {
    const fetchRadarData = async () => {
      try {
        ...
        console.log('🌧️ Fetching radar animation data...');

        const response = await fetch('https://cloud-image.vercel.app/get-latest-radar');
        ...
        if (data.data_list && Array.isArray(data.data_list) && data.data_list.length > 0) {
          ...
          console.log(`📊 Loaded ${imagesList.length} radar images for animation`);
        ...
        setError(err.message || 'Failed to load radar animation data');
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (imagesData.length > 0 && !loading && !error) {
      console.log('▶️ Starting radar animation');
  ...
  const handleImageLoad = (e) => {
    if (containerRef.current) {
      ...
      if (imgWidth > 0 && imgHeight > 0) {
        ...
        console.log(`🌧️ Radar image loaded: ${imgWidth}x${imgHeight}, calculated height: ${calculatedHeight}px`);
      }
    }
  };
